<div x-data="{ open: false, playerId: null, playerName: null}" @ratingStored.window="open = false">
    <div class="mb-4">
    ...
    <table class="table-auto w-full text-left">
        ...
        @foreach($players as $player)
            ...
            <tr class="border-b border-gray-200">
                ...
                <td><button @click="playerId = {{ $player->id }}, open = true, playerName = '{{ $fullName }}'" class="p-2 rounded-lg">
    ...
    <!-- Modal -->
    <div x-show="open" class="fixed z-50 inset-0 overflow-y-auto flex items-center justify-center" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-black opacity-50"></div>
        <!-- Modal Content -->
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 relative z-60 mx-4 sm:mx-auto w-full sm:w-3/4 lg:w-1/2 rounded-lg">
            <form x-on:submit.prevent="$wire.emit('storePlayerRecord', playerId)">
                <span class="text-xl" >Eintrag für</span> <span class="text-xl text-red-500" x-text="playerName"></span>
                <div class="mb-4">
                    <label for="rating" class="block text-gray-700 text-sm font-bold mb-2">Bewertung:</label>
                ...
                <div class="mb-4">
                    <label for="comment" class="block text-gray-700 text-sm font-bold mb-2">Kommentar:</label>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="submit" @click="open = false" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Eintragen
                    </button>
